...
public class PayPalServiceImpl implements PayPalService {
    ...
    try {
      ...
      if (responseCode != HTTP_OK) {
        throw new PaymentProcessingException(
            "Error creating order in PayPal for booking: " + inputBookingId);
      }
      ...
      throw new PaymentProcessingException(
          "Error serializing order data for booking: " + inputBookingId, e);
    } catch (JSONException | IOException e) {
      throw new PaymentProcessingException(
          "Error in communication with PayPal for booking: " + inputBookingId, e);
    }
  }
  ...
  public JSONObject capturePaymentForOrder(Integer inputBookingId, String inputOrderId) {
    ...
    try {
      HttpURLConnection httpConn =
          setUpConnection(paypalApiUrl + inputOrderId + "/capture", "POST", inputBookingId);
      ...
      if (responseCode != HTTP_OK && responseCode != HTTP_CREATED) {
        throw new PaymentProcessingException(
            "Error capturing payment in PayPal for order: " + inputOrderId);
      }
      ...
      throw new PaymentProcessingException(
          "Error in communication with PayPal for order: " + inputOrderId, e);
    }
  }

  private HttpURLConnection setUpConnection(
      String apiUrl, String requestMethod, Integer inputBookingId) throws IOException {
  ...
  private String constructRequestBody(Integer inputBookingId, Booking dbBooking)
  ...
  private void refreshToken() {
    ...
    if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
      ...
      System.err.println(
          "Failed to refresh PayPal token. Status code: " + response.getStatusCode());
    }
  }
