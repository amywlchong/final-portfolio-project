...
@Service
public class ReviewServiceImpl extends GenericServiceImpl<Review, ReviewResponseDTO>
  ...
  public void deleteById(Integer inputReviewId) {
    ...
    tourService.updateTourRatingsAfterDeletingReview(
        associatedTour.deepCopy(), reviewToBeDeleted.getRating());
        ...
        .orElseThrow(
            () ->
                new NotFoundException(
                    "Did not find booking id: "
                        + bookingId
                        + ". Review cannot be created without an associated booking."));
  }

  private void validateUser(Integer userId) {
    if (authService.verifyAuthenticatedUserHasRole(Role.ROLE_CUSTOMER)
        && !authService.verifyAuthenticatedUserHasId(userId)) {
      throw new PermissionDeniedException(
          "Customer can only create or delete reviews for their own bookings.");
    }
  }
}
