...
@Service
public class TourServiceImpl extends GenericServiceImpl<Tour, TourResponseDTO>
  ...
  @Override
  public List<TourResponseDTO> findAvailableToursWithinRange(
      LocalDate startDate, LocalDate endDate) {
    if (startDate.isAfter(endDate)) {
      throw new IllegalArgumentException("Start date cannot be after end date");
    }

    List<Tour> availableToursWithinRange =
        tourRepository.findAvailableToursWithinRange(startDate, endDate);
  ...
  private void processTourPointsOfInterestForCreate(Tour inputTour) {
    List<TourPointOfInterest> inputTourPointsOfInterest =
        CollectionUtils.nullToEmpty(inputTour.getTourPointsOfInterest());
    ...
    List<PointOfInterest> existingPOIs =
        TourUpdateProcessor.findExistingAssociatedEntitiesFromUniqueField(
    ...
    inputTourPointsOfInterest.forEach(
        tourPOI -> {
  ...
  private void processTourStartDatesForCreate(Tour inputTour) {
    List<TourStartDate> inputTourStartDates =
        CollectionUtils.nullToEmpty(inputTour.getTourStartDates());

    // get start dates that match the date-times of the input start dates from the database
    List<StartDate> existingStartDates =
        TourUpdateProcessor.findExistingAssociatedEntitiesFromUniqueField(
            inputTourStartDates,
  ...
  private void setAvailableSpaces(TourResponseDTO tour) {
    ...
    tourStartDates.forEach(
        tourStartDate -> {
          Integer totalBookedSpaces =
              bookingRepository.sumParticipantsByTourStartDate(tourStartDate);
          Integer availableSpaces =
              TourStartDateHelper.computeAvailableSpacesForStartDates(
                  tourStartDate, totalBookedSpaces);
      ...
      if (totalBookedSpaces > inputMaxGroupSize) {
        throw new IllegalArgumentException(
            "The input maxGroupSize is too small. "
                + "More than "
                + inputMaxGroupSize
                + " participants have already booked the tour for certain dates.");
      }
    }
  }
}
