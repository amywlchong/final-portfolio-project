...
const getAvailableToursWithinRange = async (
  formattedStartDate: string,
  formattedEndDate: string
) => {
  const { data } = await axios.get<Tour[]>(
    `${apiBaseUrl}/tours/available?startDate=${formattedStartDate}&endDate=${formattedEndDate}`
  );
...
const getOneTour = async (id: number) => {
  const { data } = await axios.get<Tour | undefined>(
    `${apiBaseUrl}/tours/${id}`
  );
...
const uploadTourImages = async (tourId: number, images: File[]) => {
  ...
  const authHeader = getAuthHeader();
  const { data } = await axios.post<TourImage[]>(
    imageUrl,
    formData,
    authHeader
  );
...
const updateTour = async (
  id: number,
  tourRequest: FieldValues<TourRequest>
) => {
  const authHeader = getAuthHeader();
  const { data } = await axios.put<Tour>(
    `${apiBaseUrl}/tours/${id}`,
    tourRequest,
    authHeader
  );
...
const deleteTourImage = async (tourId: number, imageId: number) => {
  const authHeader = getAuthHeader();
  const { data } = await axios.delete<string>(
    `${apiBaseUrl}/tours/${tourId}/images/${imageId}`,
    authHeader
  );
