...
export const useBookings = (
  bookingServiceFunction: () => Promise<BookingResponse[]>
) => {
  const [isLoadingBookings, setIsLoadingBookings] = useState(false);
  const [errorFetchingBookings, setErrorFetchingBookings] =
  ...
  const fetchBookings = async () => {
    ...
    const getBookingsHandler = createServiceHandler(
    ...
    if (response.success && response.data) {
      ...
      const pastBookings = response.data
        .filter(
          (booking) =>
            new Date(booking.startDateTime).getTime() < new Date().getTime()
        )
        .sort(
          (a, b) =>
