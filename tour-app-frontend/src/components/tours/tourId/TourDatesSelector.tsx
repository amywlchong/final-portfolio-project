...
const TourDatesSelector = () => {
  ...
  const [futureTourStartDates, setFutureTourStartDates] = useState<
    TourStartDate[]
  >([]);
  const [filteredTourStartDates, setFilteredTourStartDates] = useState<
    TourStartDate[]
  >([]);
  ...
  useEffect(() => {
    const futureDates =
      tour?.tourStartDates?.filter(
        (tsd) => new Date(tsd.startDate.startDateTime) > new Date()
      ) || [];
  ...
  const sortedTourStartDates = [...filteredTourStartDates].sort(
    (a, b) =>
  ...
  const currentDates = sortedTourStartDates.slice(
    indexOfFirstDate,
    indexOfLastDate
  );
  ...
  const filterDates = (dates: TourStartDate[], range: Range) => {
    return dates.filter((date) => {
      ...
      return (
        tourStartDate >= range.startDate && tourEndDate <= userEndDateEndOfDay
      );
    });
  };
    ...
    <div>
      <Grid container>
        <Grid item xs={12} md={6} lg={5}>
          <Typography variant="h2">Select Your Availability:</Typography>
        ...
        <Grid item xs={12} md={6} lg={5}>
          <div
            style={{
              ...
              width: is500AndUp ? "400px" : is400AndUp ? "350px" : "300px",
            }}
          >
            <Typography variant="h2">Start Dates:</Typography>
          ...
          <List>
            {currentDates.map((date) => (
              <ListItem
                ...
                style={{
                  width: `${
                    is500AndUp ? "380px" : is400AndUp ? "360px" : "300px"
                  }`,
                }}
              >
                <ListItemText
