...
public class TourImageServiceImpl implements TourImageService{
    ...
    @Autowired
    public TourImageServiceImpl(TourImageRepository theTourImageRepository, TourRepository theTourRepository, TourService theTourService, S3Service theS3Service, TourImageMapper theTourImageMapper, EntityLookup theEntityLookup) {
    ...
    @Override
    @Transactional
    public List<TourImageResponseDTO> uploadTourImages(Integer inputTourId, List<MultipartFile> inputFiles, String inputCoverImageName) {
    ...
    private List<TourImage> createTourImages(Tour existingTour, List<MultipartFile> files, String coverImageName) {
        return files.stream()
                .map(file -> {
