...
const SchedulesCalendar = ({ schedules }: SchedulesCalendarProps) => {
  ...
  const filteredSchedules = schedules.filter(schedule => {
    return (
      (!filterGuideName || schedule.userName?.toLowerCase().includes(filterGuideName.toLowerCase())) &&
  ...
  const calendarEvents = Array.from(groupSchedules(filteredSchedules).values())
    .map(schedulesGroup => {
      const firstSchedule = schedulesGroup[0];
      return ({
        title: getGuideNames(schedulesGroup) ? `${getGuideNames(schedulesGroup)} - ${firstSchedule.tourName}` : `Not assigned yet - ${firstSchedule.tourName}`,
