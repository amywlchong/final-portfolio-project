...
const createReview = async (reviewRequest: ReviewRequest) => {
  const authHeader = getAuthHeader();
  const { data } = await axios.post<ReviewResponse>(`${apiBaseUrl}/reviews`, reviewRequest, authHeader);
  return data;
};

const deleteReview = async (reviewId: number) => {
  const authHeader = getAuthHeader();
  const { data } = await axios.delete<ReviewResponse>(`${apiBaseUrl}/reviews/${reviewId}`, authHeader);
  return data;
};
