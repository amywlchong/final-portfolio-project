...
const useTours = () => {
  ...
  const getAllToursHandler = createServiceHandler(() => tourService.getAllTours(), {
  ...
  const getAvailableToursWithinRangeHandler = createServiceHandler((startDate, endDate) => tourService.getAvailableToursWithinRange(startDate, endDate), {
  ...
  const filterTours = async (regions: string | (string | null)[] | null, startDate: string | (string | null)[] | null, endDate: string | (string | null)[] | null) => {
    ...
    if (typeof startDate == "string" && typeof endDate == "string") {
      const response = await getAvailableToursWithinRangeHandler(startDate, endDate);
      if (response.success && response.data) {
    ...
    const filteredTours = Array.isArray(regions) && regions.length > 0
  ...
  return { loadingAllTours, allToursError, loadingAvailableTours, availableToursError, filterTours };
};
